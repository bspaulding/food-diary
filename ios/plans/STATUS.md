...
| Phase 1 — Core logging (v1) | ◐ | — | GraphQL operations + protocol-backed repositories + DesignSystem (§10) + diary list (§4) + entry form (§5) + nutrition items (§6) + recipes (§7) + nutrition targets (§8) landed; §9 still open |
...
- [x] Repositories (protocol-backed) for diary/items/recipes/search/suggestions/targets — Note: SwiftUI features (§9: profile, error/session handling) remain open; diary list, entry form, items, recipes, design system, and nutrition targets are now landed (see below).
