...
A Rust/Warp HTTP service that parses nutrition label images using vision-language model (VLM)
inference. Two VLM backends are supported, selected at runtime via env vars:
...
OpenRouter is preferred if `OPENROUTER_API_KEY`/`LLM_API_KEY` is set, otherwise it falls back to
local llama.cpp if `VLM_MODEL_PATH`/`VLM_MMPROJ_PATH` are set, otherwise the request errors.

The earlier PaddleOCR/regex-parser backend (see Known Issues #13 for the history) has been removed
in favor of the VLM path above, which scores far higher on this project's eval.
...
If no VLM env vars are set (neither OpenRouter nor local llama.cpp), `/label` requests will fail.

### VLM Benchmarks

`src/bin/vlm_benchmark.rs` runs local llama.cpp VLM backends against `test_cases.csv` / `images/`,
and `src/bin/vlm_benchmark_api.rs` does the same against hosted API models (e.g. OpenRouter). Past
results are tracked in the living summary at [`eval-results/README.md`](eval-results/README.md) —
check it before re-running an eval someone else already ran, and add to it after any new run.
...
./scripts/run-eval.sh <key>               # full 33-image eval
```

To add a new model, add a `[models.<key>]` entry to `models.toml` and run the two commands above.

`vlm_benchmark` can also be run directly with explicit paths, if you're not using the manifest —
the VLM does the full image → structured JSON extraction itself:
...
Published to `ghcr.io/bspaulding/food-diary/nutrition-fact-labeller`. VLM weights must be mounted at runtime via `VLM_MODEL_PATH` / `VLM_MMPROJ_PATH` for the local llama.cpp fallback; OpenRouter (the default) requires no weights.
