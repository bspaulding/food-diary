#!/usr/bin/env bash
# Runs the full-JSON eval harness against a model listed in ../models.toml.
# Fetches the model first if it isn't already downloaded.
#
# Usage:
...
cd "$CRATE_DIR"
echo "Building vlm_benchmark (release)..."
cargo build --release --bin vlm_benchmark
