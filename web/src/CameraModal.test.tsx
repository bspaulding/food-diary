...
describe("CameraModal", () => {
  ...
  it("should send backend=vlm when Use VLM checkbox is checked", async () => {
    const user = userEvent.setup();
    mockGetUserMedia.mockResolvedValue(mockStream);
    ...
    // Check the VLM checkbox
    const vlmCheckbox = screen.getByRole("checkbox", { name: /use vlm/i });
    await user.click(vlmCheckbox);
    expect((vlmCheckbox as HTMLInputElement).checked).toBe(true);

    // Trigger capture (will fail due to mock canvas, but the formData is still built)
    const captureButton = screen.getByText("Capture & Import");
    await user.click(captureButton);

    await waitFor(() => {
      if (capturedFormData) {
        expect(capturedFormData.get("backend")).toBe("vlm");
      }
    });
  });

  it("should not send backend field when Use VLM checkbox is unchecked", async () => {
    const user = userEvent.setup();
    mockGetUserMedia.mockResolvedValue(mockStream);
    let capturedFormData: FormData | null = null;
    global.fetch = vi.fn().mockImplementation((_url, opts) => {
    ...
    const vlmCheckbox = screen.getByRole("checkbox", { name: /use vlm/i });
    expect((vlmCheckbox as HTMLInputElement).checked).toBe(false);
    ...
    await waitFor(() => {
      if (capturedFormData) {
        expect(capturedFormData.get("backend")).toBeNull();
      }
    });
  });
